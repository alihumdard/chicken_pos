...
@section('content')
                        ...
                        <div id="wholesale-category-grid" class="category-slider flex overflow-x-auto space-x-3 pb-2">
                            @foreach ($wholesaleProducts as $p)
                                <div class="category-tab flex-shrink-0 bg-white border border-gray-200 rounded-xl p-3 flex flex-col items-center justify-center cursor-pointer w-28 h-28 lg:w-32 lg:h-32"
                                    ...
                                    <img src="{{ asset('assets/images/' . $p['img']) }}" class="w-10 h-10 object-contain mb-2">
                                    <span class="text-[10px] font-bold text-center uppercase leading-tight">{{ $p['label'] }}</span>
                                </div>
                            @endforeach
                        </div>
   ...
   {{-- 🟢 MODAL: VIEW RATES & FORMULAS (Updated Design) --}}
<div id="ratesModal" class="fixed inset-0 z-[60] hidden" role="dialog" aria-modal="true">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @foreach($wholesaleProducts as $p)
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            <td class="py-3 font-semibold text-slate-700">{{ $p['label'] }}</td>
                                            <td class="py-3 text-right">
                                                <span class="text-blue-700 font-black text-lg">
                                                    {{ number_format($rates['wholesale'][$p['field']] ?? 0, 2) }}
                                                </span>
                                        ...
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="py-3 text-right">
                                                <span class="text-green-700 font-black text-lg">
                                                    {{ number_format($rates['retail'][$p['field']] ?? 0, 2) }}
                                                </span>
                                                <span class="text-[10px] text-slate-400 font-bold ml-1">PKR</span>
                                            </td>
                                        </tr>
    ...
    <div id="contactModal" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-md border">
                ...
                <form id="contactForm">
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div>
                            ...
                            <input type="number" name="opening_balance" value="0" class="w-full px-4 py-2.5 rounded-lg border outline-none">
                        </div>
                    </div>
                    <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse gap-3">
                        <button type="submit" class="bg-slate-800 text-white px-6 py-2.5 rounded-lg font-semibold">Save</button>
...
document.getElementById('sale-form').addEventListener('submit', async function(e) {
    ...
    // Data object taiyar karein bajaye FormData ke
    const payload = {
        ...
        cart_items: cartItems // Yeh ab sahi Array format mein jayega
    };
    ...
    try {
        const res = await fetch("{{ route('admin.sales.store') }}", {
            ...
            headers: { 
                ...
                'Content-Type': 'application/json', // Content type JSON set karein
                'X-CSRF-TOKEN': payload._token
            },
            body: JSON.stringify(payload) // Poora data JSON bana kar bhejein
        });
