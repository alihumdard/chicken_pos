...
@section('content')
    ...
    <div class="w-full max-w-[100vw] min-h-screen overflow-x-hidden bg-gray-100 flex flex-col pt-5 sm:pt-0">
        ...
        <div class="flex-1 p-3 sm:p-6 lg:p-8">
            ...
            {{-- 🟢 HEADER & ADD BUTTON --}}
            {{-- Stack vertically on mobile, row on small screens and up --}}
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Poultry Management</h1>

                {{-- Button is full width on mobile for easier tapping --}}
                <button onclick="openModal()"
                    class="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-lg flex items-center justify-center transition-colors shadow-md text-sm sm:text-base">
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden w-full">
                {{-- overflow-x-auto enables horizontal scrolling SPECIFICALLY for the table --}}
                <div class="overflow-x-auto w-full">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                {{-- whitespace-nowrap prevents headers from breaking into two lines on small screens --}}
                                <th
                            ...
                            @forelse($poultries as $item)
                                <tr id="row-{{ $item->id }}" class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                        ...
                                        @if ($item->description)
                                            <span
                                                class="block text-xs text-gray-400 truncate max-w-[150px]">{{ $item->description }}</span>
                                        @endif
                                    </td>
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-right">
                                        {{ number_format($item->total_weight, 2) }} KG
                                    </td>
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-right text-blue-600">
                                        @if ($item->quantity > 0)
                                    ...
                                    <td
                                        class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-right font-bold text-green-600">
                                        {{ number_format($item->cost_price, 0) }} PKR
                                    </td>
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-center text-sm">
                                        <button onclick="editPoultry({{ $item->id }})"
                                            class="text-blue-500 hover:text-blue-700 mr-3 p-1" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button onclick="deletePoultry({{ $item->id }})"
        ...
        <div id="poultryModal" class="fixed inset-0 z-50 hidden" style="z-index: 60;">
            ...
            {{-- 
            Adjusted padding (p-4) to ensure the modal doesn't touch screen edges on mobile.
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative w-full max-w-lg bg-white rounded-xl shadow-2xl flex flex-col">
                    ...
                    {{-- Modal Header --}}
                    <div
                        class="px-4 sm:px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-xl">
                        ...
                        <form id="poultryForm">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="batch_no" id="batch_no" placeholder="Enter buyer name"
                                        class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none">
                                </div>
                                ...
                                sm:grid-cols-2 (Tablet/Desktop) -> Side by side
                                --}}
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Total Weight
                                            (KG)</label>
                                    ...
                                    <div>
                                        ...
                                        <input type="number" step="0.01" name="cost_price" id="cost_price"
                                            placeholder="0.00"
                                            class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none font-bold text-gray-800"
                                            required>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="description" id="description" rows="2"
                                        class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none resize-none"
                            ...
                            {{-- Footer Buttons --}}
                            {{-- Stack buttons on very small screens if needed, otherwise row-reverse --}}
                            <div class="mt-6 flex flex-col-reverse sm:flex-row justify-end gap-3">
                                ...
                                <button type="submit" id="submitBtn"
                                    class="w-full sm:w-auto px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow transition-colors font-medium">Save
                                    Details</button>
                            </div>
                        </form>
                    </div>
        ...
        async function editPoultry(id) {
            try {
                ...
                if (result.success) {
                    ...
                    // Check if quantity field exists in form (added in HTML updates)
