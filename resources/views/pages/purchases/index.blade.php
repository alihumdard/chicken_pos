...
@section('content')
            ...
            <div class="flex-1 px-6 pb-4 flex flex-col gap-4 overflow-y-auto">
                            ...
                            <select name="supplier_id" id="supplier_id" required
                                ...
                                @if(isset($suppliers))
                                    @foreach($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" data-name="{{ $supplier->name }}">{{ $supplier->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                ...
                <div class="bg-gray-800 p-4 rounded-2xl shadow flex flex-col">
                    ...
                    <div class="grid grid-cols-3 gap-4 flex-1">
                        ...
                        @php
                            // Define the default data structure to eliminate any potential null issues
                            $defaultFormula = ['multiply' => 1.0, 'divide' => 1.0, 'plus' => 0.0, 'minus' => 0.0];
                            $formulaData = $defaultFormula; // Initialize with default

                            try {
                                // Attempt to fetch the custom formula from the database
                                $purchaseFormula = \App\Models\RateFormula::where('rate_key', 'purchase_effective_cost')->first();

                                if ($purchaseFormula) {
                                    // If found, overwrite defaults with database values
                                ...
                                // Log the error but continue using the default formula data
                                // This prevents the page from crashing and outputs a valid default JSON string
                            }
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // This factor is only used for the legacyBusinessRule fallback (we will now change the fallback)
            const EFFECTIVE_COST_FACTOR = 1.2;
            const csrfToken = document.querySelector('input[name="_token"]').value;
            // 1. Initialize purchaseFormula based on settings data or fallback
            let purchaseFormula = null;
            const formulaDataElement = document.getElementById('purchase_formula_data');
            // Define the TRUE non-modifying default for safety
            const nonModifyingDefault = { multiply: 1.0, divide: 1.0, plus: 0.0, minus: 0.0 };

            console.log("Raw Formula Value =>", formulaDataElement.value);

            if (formulaDataElement && formulaDataElement.value) {
                try {
                    // Parse the data that was passed safely via json
                    const raw = formulaDataElement.value.trim();

                    if (raw === "" || raw === "{") {
                        purchaseFormula = nonModifyingDefault;
                    } else {
                        purchaseFormula = JSON.parse(raw);
                    }

                } catch (e) {
                    console.error("Error parsing purchase formula JSON:", e);
                    // If parsing fails despite json (extreme error case), fallback to default.
                    purchaseFormula = nonModifyingDefault;
                }
            } else {
                // If the input element itself is missing or empty, use the non-modifying default.
                purchaseFormula = nonModifyingDefault;
            }

            // 🟢 2. We only fall back if purchaseFormula is null
            if (purchaseFormula === null) {
                // Fallback 2: Data element missing/empty. Use the TRUE non-modifying default (1.0)
                purchaseFormula = nonModifyingDefault;
            }
            ...
            // HELPER FUNCTION to apply the formula (Same function as used in Rates view)
            function applyFormulaJS(baseRate, formula) {
                if (!formula) return baseRate;

                // Note: parseFloat is critical here as the values come in as strings from the hidden input.
            ...
            function calculateTotals() {
                ...
                // 3. Base Cost for Formula Application
                ...
                // 4. Apply the Formula from Settings page (Uses the full formula: *, /, +, -)
